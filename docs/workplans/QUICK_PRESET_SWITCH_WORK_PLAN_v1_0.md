...
**`prebuild` now runs SEVEN checks** — the fifth added by I-13, the sixth by I-26, the seventh by I-27.
This programme adds five instruments of its own:
...
| `check-changelog-clean.mjs` **(new, I-27)** | `npm run build` → `prebuild` | an internal engineering note sits in a RELEASED CHANGELOG section, which is published verbatim as the release body |
...
| I-27 | **Post-release** — an internal note reached the public release notes + **a seventh gate** | no | build + live | ✅ |
...
| 2026-08-26 | **⚠ OPERATOR-REPORTED, and it was a REAL DEFECT in the tour engine — pre-existing in v0.4.0.** The walkthrough card covered the very control it was pointing at. `TourProvider`’s placement said *"Never overlapping the anchor"* and that was **false**: when the card fits neither below nor above, `Math.max(16, above)` clamps its TOP to the viewport and its bottom then extends **down across** the anchor. Swept as a property rather than tested on one guessed instance — 4104 combinations of anchor position, anchor height and card height: **the old formula overlaps in 936 (22.8%), the new one in 0**. Fixed with a **side** placement when neither vertical position fits. |
| 2026-08-26 | **And my own writing is what made it show.** The two longest step bodies in the entire walkthrough were both mine — **504** and **379** characters against a median of **150**. A tall card is what drives the placement into its broken branch. Trimmed to 249 and 279; max body is now 280, median unchanged at 150. Length was never only a layout problem: **a step nobody finishes reading teaches nothing**, which is the failure this whole audit exists to prevent. |
| 2026-08-26 | **The parked Spanish terminology finding was RIGHT, and bigger than it looked.** The operator spotted the tour chapter titled *"Layouts"* in the Spanish chooser. The sweep found **25** Spanish strings using the English noun against **74** already using *Diseño* — one concept with two names, the same defect class as one value carrying two meanings. All 25 corrected; *Diseño* now in 98 strings, "Layout" in **zero**. It had been parked as pre-existing; the operator seeing it is what made it in scope, and the sweep is why it was 25 rather than 1. |
| 2026-08-26 | **⛔ THE HEREDOC BACKSPACE TRAP CAUGHT ME AGAIN — in the very session that recorded it, and the bite test is the only reason I know.** A terminology rule was added to the parity gate so this class cannot drift back. Its control passed. Then the bite test **reintroduced the operator’s exact defect and the gate said OK**. The regex had been written through a heredoc and its `\b` word boundaries arrived as literal **0x08 BACKSPACE** bytes — invisible to `grep`, which rendered it as a plausible `/Layouts?/`. Handbook §10, reproduced verbatim: *a gate that reports OK while matching nothing, ever*. Repaired by building the escape from `chr(92)`, verified at byte level (0 control bytes), then re-bitten: the gate now FAILS on the defect and stops the build before `tsc`/`vite` run. **Had I trusted the passing control, I would have shipped a decorative gate against a defect the operator had just reported.** |
| 2026-08-26 | **OPEN QUESTION CLOSED — no defect.** During the sweep the Sandbox’s Layout A changed from *USDJ Jubilee Observatory* (containing the multi-window entry) to *Monitor 4 - Basic Layout* with five plain entries, and every Quick Preset vanished. That is the exact silhouette of the data loss I-15 was built to prevent, so it was raised rather than assumed benign. **The operator confirms they erased every existing Layout and built a new one deliberately.** Nothing was lost by the app. I-15’s fix stands on its earlier byte-identical proof. Recorded because the alternative reading would have stopped the release, and the difference between the two was a question, not an inspection. |
| 2026-08-26 | **Operator-reported: the bottom bar’s buttons burst out of it in Spanish. Pre-existing in v0.4.0, and NOT caused by my edit** — the diff shows I only moved `mt-4` to a wrapper; `h-12` was already there. The bar was pinned to **48 px** while Spanish labels run up to **13 characters longer** (*"Borrar todas las cuadrículas"* vs *"Clear All Grids"*), so they wrap to two lines and overflow. Fixed structurally — `min-h-12` + `flex-wrap` + `py-1` — not by shortening the Spanish, which would only hide it until the next language. English is unchanged, since its content already fits 48 px, and the construct’s `flex-1 min-h-0` middle row yields the space. **The codebase already knew this remedy**: `MonitorSelector` uses `min-h-8` with a comment naming the Spanish label that forced it. The lesson had been learned in one component and not carried to the next. |
| 2026-08-26 | **The sweep for that defect class over-reported, and narrowing it mattered.** A first pass flagged **52** fixed-height elements containing translated text — but a fixed height on a *button* or an *input* is correct and deliberate. Narrowed to **flex containers with a fixed height and no `min-h`**: **5**, of which three are single-line inputs and a pill. Two multi-word buttons were given `min-h` as insurance. **The bottom bar was the only genuine instance.** Reporting 52 would have been alarming and wrong. |
...
| 2026-08-27 | **v0.5.0 SHIPPED — and the first line users read was an internal engineering note.** The published release body and the updater manifest both opened with *"Sandbox-validated (RELEASING.md §3.5) — build `0.4.0-sb.1787788836748`…"*. **Nobody was careless: the procedure instructed it.** `RELEASING.md §3.5` step 4 says record the sandbox pass in the CHANGELOG `[Unreleased]` notes, and `[Unreleased]` is precisely the section `bump-version.mjs` rolls into the **public release body**. Two steps of one document, each sensible alone, that together publish an internal record **every release**. Fixed at the root — §3.5 now directs the record to the work plan and says why — and gated (`check-changelog-clean.mjs`, bitten on the live defect: 3 markers on the shipped line, then clean across 37 version sections). **Two public surfaces, not one.** Editing the release body left `latest.json` untouched, and `latest.json` is the **more** visible of the two — its `notes` are what every 0.4.0 user sees in the in-app update prompt. Patched by hand so the 420-char signature, url, version and pub_date stayed byte-identical; only `notes` changed. **⚠ And the verification instrument lied.** `curl` on both the `latest/download` alias *and* the direct asset URL kept returning the OLD notes after a successful upload — GitHub CDN caching. The authoritative readings disagreed with it: the API reported the asset at **3850 bytes** against the original **4007**, matching my patched file exactly, and an API download confirmed it clean. **Had I trusted `curl`, I would have concluded the fix failed and re-uploaded repeatedly against a cache.** Two independent readings, and the one that was easiest to run was the wrong one. |
